...
apci.plot.hexagram <- function(model, #matrix: age as rows, period as columns first_age,
  ...
  data <- data.table::dcast(data.table::as.data.table(data),age~period,value.var = "value")%>%
    as_tibble%>%.[,-1]%>%as.matrix
  nnrow <- nrow(data)
  nncol <- ncol(data)
  ...
  data <- cut(data,quantile(data,probs = seq(0,1,quantile)),
                include.lowest = T,
                labels = quantile(data,probs = seq(0,1,quantile))[-1])
  }
...
if(!is.null(quantile)){
  data$value <- cut(data$value,quantile(data$value,probs = seq(0,1,quantile)),
                    include.lowest = T,
                    labels = quantile(data$value,probs = seq(0,1,quantile))[-1])
  ...
  g <- ggplot(data,aes(x=period,y=age,fill=value))+
  geom_tile()+
...
run <- lapply((1:nrow(model$cohort_average))[model$cohort_average$sig!="   "],
 ...
 g<<-g+geom_abline(intercept = intercept,slope = 1,color = "green",linetype='dotted')
         }else{
  ...
  g<<-g+geom_abline(intercept = intercept,slope = 1,color = "green",linetype='dotted')
         ...
         intercept <- model$cohort_average$cohort_index[i]
  g<<-g+geom_abline(intercept = intercept,
...
apci.plot.raw <- function(data,
  ...
  g1 <- ggplot(data,aes(x=period,group=age,y = outcome_var,col=age))+
    geom_point()+
    geom_path()+
    # ylim(c(0, 1))+
    theme_bw()+
    labs(x = "Period Group",names = "Age",y = as.character(outcome_var))+
    geom_point(data = data %>%
                group_by(period) %>%
                summarise(outcome_var = mean(outcome_var,na.rm = T)),
              mapping = aes(x = period,group=NA,y=outcome_var,col=NA),
              size = 3,shape=8,color="black")

 g2 <- ggplot(data,aes(x=age,group=period,y = outcome_var,col=period))+
    geom_point()+
    geom_path()+
    # ylim(c(0, 1))+
    theme_bw()+
    labs(x = "Age Group",names = "Period",y = as.character(outcome_var))+
   geom_point(data = data %>%
               group_by(age) %>%
               summarise(outcome_var = mean(outcome_var,na.rm = T)),
             mapping = aes(x = age,group=NA,y=outcome_var,col=NA),
             size = 3,shape=8,color="black")
...
apci.plot <- function(model,
  ...
  g2 <- ggplot(data,aes(x=age,group=period,y = outcome_var,col=period))+
    ...
    geom_point(data = data %>%
              ...
              mapping = aes(x = age,group=NA,y=outcome_var,col=NA),
              size = 3,shape=8,color="black")
              # size = 3, linetype='dashed',alpha = 0.5)+
  g3 <- ggplot(data,aes(x=period,y=age,fill=outcome_var))+
    geom_tile()+
    coord_equal()+
    theme_bw()+
    ...
    labs(x = 'Period Group',
         y = 'Age Group')
  g4 <- ggplot(data,aes(x=age,y=outcome_var))+
    theme_void()+
    geom_text(x=0.5,y=0.5,
              label=c("Data Exploration"))

  g <- ggpubr::ggarrange(g3,g1+coord_flip(),g2,g4,
                    labels = c("C", "A","P"),
                    ncol = 2, nrow = 2)
  }
  ...
  g3 <- apci.plot.heatmap(model=model,
                          age = age,period = period,quantile = quantile,
  ...
  g4 <- ggplot(data,aes(x=age_group,group=NA,y = age_estimate))+
    theme_void()+
    geom_text(x=0.5,y=0.5,
              label=c("APC-I Model"))
  g5 <- ggplot(data,aes(x=age_group,group=NA,y = age_estimate))+
    geom_point()+
    geom_line()+
    labs(x = "Age Group",
  ...
  g6 <- ggplot(data,aes(x=period_group,group=NA,y = period_estimate))+
    geom_point()+
    geom_line()+
    labs(x = "Period Group",
  ...
  g <- ggpubr::ggarrange(g3,g5+coord_flip(),g6,g4,
                    labels = c("C", "A","P"),
                    ncol = 2, nrow = 2)
  }
  g
